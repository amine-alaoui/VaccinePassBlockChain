    ...
    private void setBarCode(final SparseArray<Barcode> barCode){
        textViewBarCodeValue.post(new Runnable() {
            ...
            public void run() {
                ...
                String information = textViewBarCodeValue.getText().toString();
                String [] str = information.split(";");
                String HashInformation = StringUtil.applySha256(information);
                int id = Integer.parseInt(str[0]);
                Block block = MainActivity.lst.get(id);
                String [] tab = block.data.split("@");
                int v = 0;
				for (String i: tab) {
					if (HashInformation.equals(i)){
						v=1;
						Intent monIntent = new Intent(getApplicationContext(),ResultActivity.class);
						monIntent.putExtra("clé",information);
						startActivity(monIntent);
					}
				}
				if (v==0) {
					Intent monIntent1 = new Intent(getApplicationContext(), InvalideActivity.class);
